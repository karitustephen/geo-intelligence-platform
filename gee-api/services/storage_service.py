...
class StorageService:
    ...
    async def upload_geotiff(self, file, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Use streaming upload to prevent memory exhaustion
            await asyncio.to_thread(blob.upload_from_file, file.file, content_type=file.content_type, rewind=True)
            
            # Store metadata in BigQuery (Blocking call moved to thread)
    ...
    async def get_analysis(self, analysis_id: str, user_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            query_job = await asyncio.to_thread(self._bq_client.query, formatted_query, job_config=job_config)
